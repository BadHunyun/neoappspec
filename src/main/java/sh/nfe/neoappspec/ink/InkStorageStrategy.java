...
public class InkStorageStrategy implements ExternalStorageStrategy {
    ...
    public InkStorageStrategy(ServerLevel level, BlockPos fromPos, Direction fromSide) {
        this.level = level;
        this.pos = fromPos;
    ...
    public @Nullable MEStorage createWrapper(boolean extractableOnly, Runnable injectOrExtractCallback) {
        if (level.getBlockEntity(pos) instanceof InkStorageBlockEntity<?> inkStorageBlockEntity) {
            return new InkStorageAdapter(inkStorageBlockEntity, injectOrExtractCallback);
        }
        return null;
    }
